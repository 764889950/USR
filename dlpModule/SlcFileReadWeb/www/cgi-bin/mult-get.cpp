...
void OpenSLC(const char file_dir[])//逐层顺序进行解析，解析完阻塞暂停，如需继续解析需解除阻塞
{
	int dx=0,dy=0;	
...
void OpenSLC_1(const char file_dir[],long layer)//指定的单层解析
{
	printf("file_dir=%s\r\nlayer=%ld\r\n",file_dir,layer);
	// return;
...
void OpenSLCToResult(const char fileSourceDir[],const char fileDesDir[])//解析文件全部内容，并在指定路径下生成执行文件
{
...
char* cjson_analysisState(const char *order_str, const char * key, char* value)//value作为输出返回
{
	cJSON *root;
...
int cjson_ParameterAnalysis(const char *order_str,char* fileNameDirOut)//value作为输出返回
{
	cJSON *root;
	root = cJSON_Parse((const char*)order_str);
	if(!root)
...
int main(void){
    ...
    {
		if(cjson_ParameterAnalysis(poststr,fileNameDirOut) != -1)
		{
...
{"fileToResult": "/home/jhon/share/USR-master/dlpModule/SlcFileReadWeb/www/cgi-bin/bLayer130.slc",
"fileNameDirOut":"/home/jhon/share/USR-master/dlpModule/SlcFileReadWeb/www/cgi-bin/bLayer130.bmp",
"pixel_bits_x":1920,
"pixel_bits_y":1080,
"zoom_x":20,
"zoom_y":20,
